...
void kvm_pagetablemap(pagetable_t pgtbl) {
...
kvminit()
{
  kernel_pagetable = kvm_newpagetable();
  // change the position
...
void
kfree_kernelpgtbl(pagetable_t pagetable)
{
  // there are 2^9 = 512 PTEs in a page table.
  for(int i = 0; i < 512; i++){
    pte_t pte = pagetable[i];
    ...
    if((pte & PTE_V) && (pte & (PTE_R|PTE_W|PTE_X)) == 0){
      // this PTE points to a lower-level page table.
      kfree_kernelpgtbl((pagetable_t)child);
      pagetable[i] = 0;
    }
  }
  kfree((void*)pagetable);
...
//freewalk
int 
printtable(pagetable_t pagetable, int depth) {
    // there are 2^9 = 512 PTEs in a page table.
  for(int i = 0; i < 512; i++){
    pte_t pte = pagetable[i];
    if((pte & PTE_V) && (pte & (PTE_R|PTE_W|PTE_X)) == 0){
      ...
      uint64 child = PTE2PA(pte);
      printtable((pagetable_t)child,depth+1);
    }
    if(pte & PTE_V){
      // print
      printf("..");
